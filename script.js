...
function showHotelDetail(id) {
  ...
  document.getElementById('detail-content').innerHTML = `
    ...
    ${rooms.map((r,i) => `<div class="room-card"><div><strong>${r.name}</strong><br><span style="color:var(--green);font-weight:600">₦${Number(r.price).toLocaleString()} / night</span></div><button class="btn-primary" onclick="bookRoom(${hotel.id},${i})">Request Booking</button></div>`).join('')}
...
function bookRoom(hotelId, roomIndex) {
  ...
  bookings.push({
    id: Date.now(), hotelId: hotel.id, hotelName: hotel.name,
    room: room.name, price: room.price, guestName: currentUser.name,
    phone: currentUser.phone, status: 'requested', date: new Date().toLocaleString()
  });
...
function submitHotel() {
  ...
  if (!name || !type || !price || !distance || !description || !contact) {
    return alert('Please fill in all required fields');
  }

  pendingHotels.push({
    id: Date.now(), name, type, price: Number(price), distance, description,
    amenities, image, contact, status: 'pending', date: new Date().toLocaleString()
  });
  ...
  document.getElementById('new-hotel-price').value = '';
...
function approveHotel(id) {
  ...
  extraHotels.push({
    ...
    price: pending.price, rating: 4.5, reviews: 0,
    distance: pending.distance, amenities: pending.amenities,
    image: pending.image, description: pending.description,
    rooms: [{ name: 'Standard Room', price: pending.price }]
  });
...
function updateAdmin() {
  ...
  pendingBox.innerHTML = pendingHotels.length ? pendingHotels.map(h => `
    <div style="border:1px solid var(--border);padding:1rem;border-radius:10px;margin-bottom:1rem">
      <strong>${h.name}</strong> · ${h.type} · ₦${Number(h.price).toLocaleString()}<br>
      ${h.distance}<br>
