const defaultHotels = [
  {id:1,name:"Afro View Hotel",code:"AFRO",type:"business",price:28000,rating:4.6,reviews:128,distance:"5 min from Asaba city centre",amenities:["Wi-Fi","Parking","Breakfast","Pool"],image:"https://images.unsplash.com/photo-1566073771259-6a8506099945?w=600",description:"Modern business hotel with reliable Wi-Fi and conference facilities.",rooms:[{name:"Standard Room",price:25000},{name:"Deluxe Room",price:28000},{name:"Executive Suite",price:45000}]},
  {id:2,name:"Delta Pearl Guest House",code:"DELTA",type:"family",price:22000,rating:4.3,reviews:86,distance:"8 min from city centre",amenities:["Wi-Fi","Pool","Breakfast","Parking"],image:"https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?w=600",description:"Comfortable family-friendly guest house with spacious rooms and a pool.",rooms:[{name:"Family Room",price:22000},{name:"Double Room",price:18000}]},
  {id:3,name:"Asaba Grand Suites",code:"GRAND",type:"luxury",price:55000,rating:4.8,reviews:204,distance:"Near major event venues",amenities:["Wi-Fi","Pool","Breakfast","Restaurant","Parking"],image:"https://images.unsplash.com/photo-1571896349842-33c89424de2d?w=600",description:"Premium suites ideal for events and special occasions.",rooms:[{name:"Deluxe Suite",price:55000},{name:"Presidential Suite",price:95000}]}
];

let extraHotels = JSON.parse(localStorage.getItem('cn7_extra_hotels') || '[]');
let hotels = defaultHotels.concat(extraHotels);
...
function refreshHotels() {
  ...
  hotels = defaultHotels.concat(extraHotels);
}
...
function searchHotels() {
  ...
  refreshHotels();
  const filtered = (!location || location === 'asaba') ? hotels : [];
  showSection('hotels');
  renderHotels(filtered);
  if (!filtered.length) {
    document.getElementById('hotel-list').innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:3rem;color:var(--muted)"><h3>No hotels found</h3><p>Currently CN7 only operates in Asaba.</p><button class="btn-primary" style="margin-top:1rem" onclick="document.getElementById(\'search-location\').value=\'Asaba\';searchHotels()">Show Asaba Hotels</button></div>';
  }
}
...
function applyFilters() {
  ...
  renderHotels(hotels.filter(h => {
    ...
    if (wifi && h.amenities.indexOf('Wi-Fi') === -1) return false;
    if (pool && h.amenities.indexOf('Pool') === -1) return false;
    if (breakfast && h.amenities.indexOf('Breakfast') === -1) return false;
    if (parking && h.amenities.indexOf('Parking') === -1) return false;
    return true;
  }));
}
...
function renderHotels(list) {
  ...
  container.innerHTML = list.map(h =>
    ...
    '<div class="hotel-card-body"><h3>' + h.name + '</h3>' +
    ...
    '<div class="verified">Verified by CN7</div></div></div>'
  ).join('');
}
...
function showHotelDetail(id) {
  ...
  document.getElementById('detail-content').innerHTML =
    ...
    '<h2>' + hotel.name + '</h2>' +
    '<div style="color:var(--muted);margin:.5rem 0 1rem">★ ' + hotel.rating + ' · ' + hotel.reviews + ' reviews · ' + hotel.type + '</div>' +
    ...
    rooms.map(function(r, i) {
      return '<div class="room-card"><div><strong>' + r.name + '</strong><br><span style="color:var(--green);font-weight:600">₦' + Number(r.price).toLocaleString() + ' / night</span></div><button class="btn-primary" onclick="bookRoom(' + hotel.id + ',' + i + ')">Request Booking</button></div>';
    }).join('') +
    '<p style="margin-top:1.5rem;font-size:.9rem;color:var(--muted)">Note: This is a booking request. Payment is made at the hotel.</p>';
}

function bookRoom(hotelId, roomIndex) {
  if (!currentUser) { alert('Please Sign In first'); toggleAuth(); return; }
  refreshHotels();
...
function renderHotelBookings() {
  ...
  list.innerHTML = hotelBookings.map(b => {
    const actions = b.status === 'requested'
      ? '<div style="margin-top:.8rem"><button class="btn-primary" style="padding:.4rem .9rem;font-size:.85rem" onclick="updateBooking(' + b.id + ',\'confirmed\')">Confirm</button><button style="background:#dc2626;color:#fff;border:none;padding:.4rem .9rem;border-radius:8px;margin-left:.5rem;cursor:pointer" onclick="updateBooking(' + b.id + ',\'rejected\')">Reject</button></div>'
      : '';
    return '<div style="border:1px solid var(--border);padding:1rem;border-radius:10px;margin-bottom:1rem"><strong>' + b.guestName + '</strong> · ' + b.phone + '<br>' + b.room + '<br>₦' + Number(b.price).toLocaleString() + ' · <span class="status ' + b.status + '">' + b.status + '</span><br><small>' + b.date + '</small>' + actions + '</div>';
  }).join('');
}

function updateBooking(id, status) {
  bookings = bookings.map(b => b.id === id ? Object.assign({}, b, {status: status}) : b);
  localStorage.setItem('cn7_bookings', JSON.stringify(bookings));
  renderHotelBookings();
  updateAdmin();
}
...
function submitHotel() {
  ...
  const rooms = [];
  document.querySelectorAll('#room-fields .room-row').forEach(function(row) {
  ...
  if (!name || !type || !distance || !description || !contact) return alert('Please fill in all required fields');
  if (!rooms.length) return alert('Please add at least one room and price');

  pendingHotels.push({
    id: Date.now(), name: name, type: type, price: Math.min.apply(null, rooms.map(r => r.price)),
    distance: distance, description: description, amenities: amenities, image: image,
    contact: contact, rooms: rooms, status: 'pending', date: new Date().toLocaleString()
  });
  ...
  document.querySelectorAll('.amenity-checks input').forEach(function(i) { i.checked = false; });
  document.getElementById('room-fields').innerHTML = '<div class="room-row"><input type="text" class="auth-input room-name" placeholder="Room type" value="Classic Room"><input type="number" class="auth-input room-price" placeholder="Price (₦)"></div>';
}

function approveHotel(id) {
  const pending = pendingHotels.find(h => h.id === id);
  if (!pending) return;
  const code = (pending.name.replace(/[^A-Za-z]/g, '').slice(0, 5) || 'HOTEL').toUpperCase();
  extraHotels.push({
    id: pending.id, name: pending.name, code: code, type: pending.type, price: pending.price,
    rating: 4.5, reviews: 0, distance: pending.distance, amenities: pending.amenities,
    image: pending.image, description: pending.description,
...
function updateAdmin() {
  ...
  pendingBox.innerHTML = pendingHotels.length ? pendingHotels.map(h =>
    '<div style="border:1px solid var(--border);padding:1rem;border-radius:10px;margin-bottom:1rem"><strong>' + h.name + '</strong> · ' + h.type + ' · From ₦' + Number(h.price).toLocaleString() + '<br>' +
    h.distance + '<br>Rooms: ' + (h.rooms ? h.rooms.map(r => r.name + ' (₦' + Number(r.price).toLocaleString() + ')').join(', ') : 'N/A') +
    '<br>Amenities: ' + (h.amenities.join(', ') || 'None') + '<br>Contact: ' + h.contact + '<br><small>' + h.date + '</small>' +
    '<div style="margin-top:.8rem"><button class="btn-primary" style="padding:.4rem .9rem;font-size:.85rem" onclick="approveHotel(' + h.id + ')">Approve</button>' +
    '<button style="background:#dc2626;color:#fff;border:none;padding:.4rem .9rem;border-radius:8px;margin-left:.5rem;cursor:pointer" onclick="rejectHotel(' + h.id + ')">Reject</button></div></div>'
  ).join('') : '<p style="color:var(--muted)">No pending applications.</p>';
